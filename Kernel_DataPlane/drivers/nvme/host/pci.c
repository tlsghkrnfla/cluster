...
release_iod:
	...
	__asm__ volatile("rdtsc" : "=a" (lo), "=d" (hi));
	value = ((unsigned long long)hi << 32) | lo;
	trace_req_completion_out(value);
...
static int nvme_queue_rq_temp(struct blk_mq_hw_ctx *hctx,
			 const struct blk_mq_queue_data *bd)
{
	unsigned int hi, lo;
	unsigned long long value;
	...
	__asm__ volatile("rdtsc" : "=a" (lo), "=d" (hi));
	value = ((unsigned long long)hi << 32) | lo;
	trace_nvme_sys_in(value);

	ret = nvme_queue_rq(hctx, bd);

	__asm__ volatile("rdtsc" : "=a" (lo), "=d" (hi));
	value = ((unsigned long long)hi << 32) | lo;
	trace_nvme_sys_out(value);
	return ret;
}
